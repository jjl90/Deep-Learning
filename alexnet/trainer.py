...
class AlexNetLargeKernel(nn.Module):
  ...
  def __init__(self, configs):
    super(AlexNetLargeKernel, self).__init__()
    self.features = nn.Sequential(
        nn.Conv2d(3, 96, kernel_size=21, stride=8, padding=1), #1
        nn.ReLU(inplace=True), #2
        nn.Conv2d(96,256,kernel_size=7, stride=2, padding=2), #3
...
class AlexNetDilation(nn.Module):
  ...
  def __init__(self, configs):
      super(AlexNetDilation, self).__init__()
      self.features = nn.Sequential(
...
#note the output dim was changed of AlexNefrom 4 to 7 respectively to do this test.
def analyze_model_kernels():
  ...
  #domain
  model_domain = AlexNet(configs)
  model_domain.load_state_dict(torch.load('experiments/P3_domain/domain_lr_0.001.wd_0.0/best_model.pt'))
  for i in range(5):
    kernels_domain = model_domain.features[arr[i]].weight.detach()
    name = "conv2D_"+str(i)+"_domain"
    visualize_kernels(name, kernels_domain)
  
  # model_category = AlexNet(configs)
  # model_category.load_state_dict(torch.load('experiments/p3_category/category_lr_0.001.wd_0.0/best_model.pt'))
  # for i in range(5):
  #   kernels_category = model_category.features[arr[i]].weight.detach()
  #   name = "conv2D_"+str(i)+"_category"
  #   visualize_kernels(name, kernels_category)
