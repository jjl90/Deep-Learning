  ...
  try:
    for epoch in range(FLAGS.epochs):
      for phase in ('train', 'eval'):
        ...
        loss_graph_set.append(epoch_loss)
        accuracy_graph_set.append(epoch_acc)
        ...
        if phase == 'eval':
          if epoch_acc > best_acc:
            best_acc = epoch_acc
            val_accuracy_graph_set.append(best_acc)
            best_model = copy.deepcopy(model.state_dict())
            torch.save(best_model, os.path.join(experiment_name,
                                                'best_model.pt'))
    print(loss_graph_set)
    print(accuracy_graph_set)
    print(val_accuracy_graph_set)
